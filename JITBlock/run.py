...
from JITBlock.model import Model
from JITBlock.my_util import convert_examples_to_features, TextDataset, eval_result, preprocess_code_line, \
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.epochs):
        ...
        for step, batch in enumerate(bar):
            (inputs_ids, input_masks, manual_features, labels) = [x.to(args.device) for x in batch]
            model.train()
            loss, logits,_= model(inputs_ids, input_masks, manual_features, labels, train_dataset.weight_dict)
            if args.n_gpu > 1:
                loss = loss.mean()
            ...
            if (step + 1) % args.save_steps == 0:
                ...
                if results['eval_f1'] > best_f1:
                    ...
                    if args.do_ablation:
                        checkpoint_prefix = f'checkpoint-best-f1_{args.max_changed_block_unit}_ablation'
                    else:
                        checkpoint_prefix = f'checkpoint-best-f1_{args.max_changed_block_unit}'
...
def evaluate(args, model, tokenizer, eval_when_training=False):
    # build dataloader
    if args.do_ablation:
        cache_dataset = os.path.dirname(args.eval_data_file[0]) + f'/valid_set_cache_msg{args.max_msg_length}_{args.max_changed_block_unit}_ablation.pkl'
    else:
        cache_dataset = os.path.dirname(args.eval_data_file[0]) + f'/valid_set_cache_msg{args.max_msg_length}_{args.max_changed_block_unit}.pkl'
    ...
    for batch in tqdm(eval_dataloader):
        (inputs_ids, input_masks, manual_features, labels) = [x.to(args.device) for x in batch]
        with torch.no_grad():
            loss, logit, _, = model(inputs_ids, input_masks, manual_features, labels, eval_dataset.weight_dict)
...
def test(args, model, tokenizer, best_threshold=0.5):
    # build dataloader
    if args.do_ablation:
        cache_dataset = os.path.dirname(args.test_data_file[0]) + f'/test_set_cache_msg{args.max_msg_length}_{args.max_changed_block_unit}_ablation.pkl'
    else:
        cache_dataset = os.path.dirname(args.test_data_file[0]) + f'/test_set_cache_msg{args.max_msg_length}_{args.max_changed_block_unit}.pkl'
    logger.info("Cache Dataset file at %s ", cache_dataset)
    if os.path.exists(cache_dataset):
        eval_dataset = pickle.load(open(cache_dataset, 'rb'))
    else:
    ...
    for batch in tqdm(eval_dataloader):
        ...
        with torch.no_grad():
            loss, logit, attn_weights = model(inputs_ids, attn_masks, manual_features, labels, eval_dataset.weight_dict,output_attentions=True)
    ...
    for example, pred, prob, attn in zip(eval_dataset.examples, y_preds, logits[:, -1], attns):
        ...
        # calculate
        if int(example.label) == 1 and int(pred) == 1 and '[ADD]' in [token for input_token in example.input_tokens for token in input_token ]:
    ...
    RF_result.to_csv(os.path.join(args.output_dir, f"predictions_{args.max_changed_block_unit}.csv"), sep='\t', index=None)
...
def deal_with_attns(item, attns, pred, commit2codes, idx2label, only_adds=False,attn_index=0):
    '''
    ...
    input_tokens =[token for input_token in item.input_tokens for token in input_token ]
    ...
    start=0
    while True:
        if '[ADD]' in input_tokens[start:]:
            begin_pos = input_tokens.index('[ADD]',start)
        else:
            break
        end_pos = input_tokens.index('[DEL]',begin_pos) if '[DEL]' in input_tokens[begin_pos:] else len(input_tokens) - 1
        add_tokens+=input_tokens[begin_pos:end_pos]
        add_tokens_index+=[index for index in range(begin_pos,end_pos)]
        if end_pos == len(input_tokens)-1:
            break;
        else:
            start = end_pos
    ...
    attns =attns[np.array(add_tokens_index)]
    attn_df['score'] = attns
...
def parse_args():
    ...
    parser.add_argument("--max_changed_block_unit", type=int, default=1)

    parser.add_argument("--do_ablation", type=bool, default=False)
...
def main(args):
    ...
    if args.do_eval:
        if args.do_ablation:
            checkpoint_prefix = f'checkpoint-best-f1_{args.max_changed_block_unit}_ablation/model.bin'
        else:
            checkpoint_prefix = f'checkpoint-best-f1_{args.max_changed_block_unit}/model.bin'
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))
        checkpoint = torch.load(output_dir)
    ...
    if args.do_test:
        ...
        eval_result(os.path.join(args.output_dir, f"predictions_{args.max_changed_block_unit}.csv"), args.test_data_file[-1])
