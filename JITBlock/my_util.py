...
class InputFeatures(object):
    ...
    def __init__(self, commit_id, input_ids, input_tokens, input_masks, label, manual_features):
...
def convert_examples_to_features(item, cls_token='[CLS]', sep_token='[SEP]', sequence_a_segment_id=0,
    ...
    if args.do_ablation:
        ...
        all_input_token = msg_tokens + ['[ADD]'] + added_tokens + ['[DEL]'] + removed_tokens

        for i in range(0, len(all_input_token), 512):
            input_token = all_input_token[i:i + 512 - 2]
            input_token = [tokenizer.cls_token]  + input_token + [tokenizer.sep_token]
            ...
            input_token = input_token + [tokenizer.pad_token] * padding_length
            ...
            if len(input_tokens) == args.max_changed_block_unit:
                break
    else:
        input_token = msg_tokens
        changed_blocks = []
        for changed_block in file_codes:
            changed_str = []
            for i in range(0, len(changed_block), 2):
                sub_str = preprocess_code_line(' '.join(changed_block[i + 1].split()))
                if len(sub_str.replace(' ', '')) == 0:
                    continue
                if changed_block[i] == 'added_code':
                    changed_str.append(' '.join(['[ADD]', sub_str]))
                if changed_block[i] == 'removed_code':
                    changed_str.append(' '.join(['[DEL]', sub_str]))
            if len(changed_str):
                changed_blocks.append(' '.join(changed_str))
        for i, changed_block in enumerate(changed_blocks):
            next_len = len(input_token + tokenizer.tokenize(changed_block))
            if next_len <= 509:
                input_token = input_token + [tokenizer.sep_token] + tokenizer.tokenize(changed_block)
            if next_len > 509 or (i == len(changed_blocks) - 1 and len(input_tokens) == 0):
                input_token = input_token[:512 - 2]
                input_token = [tokenizer.cls_token] + input_token + [tokenizer.sep_token]
                input_id = tokenizer.convert_tokens_to_ids(input_token)
                input_mask = [1 if mask_padding_with_zero else 0] * len(input_id)
                ...
                input_token = msg_tokens +[tokenizer.sep_token]+tokenizer.tokenize(changed_block)
                if len(input_tokens) == args.max_changed_block_unit:
                    break

    for _ in range(args.max_changed_block_unit - len(input_tokens)):
        input_id = [tokenizer.pad_token_id] * 512
        input_token = [tokenizer.pad_token] * 512
        input_mask = [0 if mask_padding_with_zero else 1] * 512
        input_tokens.append(input_token)
        input_ids.append(input_id)
        input_masks.append(input_mask)
...
def convert_dtype_dataframe(df, feature_name):
    df['fix'] = df['fix'].apply(lambda x: float(x == 'True'))
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path=None, mode='train'):
        ...
        sample_weights = compute_class_weight(class_weight='balanced', classes=np.unique(labels), y=labels)
        ...
        self.examples = [convert_examples_to_features(x) for x in
    ...
    def __getitem__(self, item):
        return (torch.tensor(self.examples[item].input_ids),
                torch.tensor(self.examples[item].input_masks),
